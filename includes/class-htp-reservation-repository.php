...
final class HTP_Reservation_Repository implements HTP_Reservation_Repository_Interface {
	public function find_active( $product_id, $user_id ) {
		$ids = get_posts( array(
			...
			'meta_query' => array(
				array( 'key' => HTP_Reservation_Meta::PRODUCT_ID, 'value' => absint( $product_id ), 'type' => 'NUMERIC' ),
	...
	public function count_active( $user_id = 0, $email = '' ) {
		...
		if ( $user_id > 0 ) {
			...
			$args['meta_query'][] = array( 'key' => HTP_Reservation_Meta::EMAIL, 'value' => sanitize_email( $email ) );
		} else {
			return 0;
		}
	...
	public function count_open( $user_id ) {
		...
		$query = new WP_Query( array(
			...
			'meta_query' => array(
				'relation' => 'OR',
				array( 'relation' => 'AND', array( 'key' => HTP_Reservation_Meta::STATUS, 'value' => HTP_Reservation_Status::PENDING ), array( 'key' => HTP_Reservation_Meta::EXPIRES_AT, 'value' => time(), 'type' => 'NUMERIC', 'compare' => '>' ) ),
				array( 'relation' => 'AND', array( 'key' => HTP_Reservation_Meta::STATUS, 'value' => HTP_Reservation_Status::ACTIVE ), array( 'key' => HTP_Reservation_Meta::EXPIRES_AT, 'value' => time(), 'type' => 'NUMERIC', 'compare' => '>' ) ),
			),
		) );
		return (int) $query->found_posts;
	}
	...
	public function user_has_open_for_product( $product_id, $user_id ) {
		...
		$ids = get_posts( array(
			...
			'author' => absint( $user_id ),
			'meta_query' => array(
				array( 'key' => HTP_Reservation_Meta::PRODUCT_ID, 'value' => absint( $product_id ) ),
				array( 'key' => HTP_Reservation_Meta::STATUS, 'value' => HTP_Reservation_Status::open(), 'compare' => 'IN' ),
			),
		) );
		$now = time();
		foreach ( $ids as $reservation_id ) {
			if ( (int) HTP_Reservation_Meta::get( $reservation_id, HTP_Reservation_Meta::EXPIRES_AT ) > $now ) return true;
		}
		return false;
	}
}
